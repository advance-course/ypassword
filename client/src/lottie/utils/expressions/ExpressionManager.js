...
function initiateExpression(elem, data, property) {
  ...
  if (data.xf) {
    ...
    for (i = 0; i < len; i += 1) {
      ...
      __expression_functions[i] = _interpreterWrap2.default.run('module.exports = ' + data.xf[i]);
    }
  }
  ...
  function seedRandom(seed) {
    _common2.default.seedrandom(randSeed + seed);
  }
  ...
  function executeExpression(_value) {
    ...
    if (scoped_bm_rt.propType === 'shape') {
      scoped_bm_rt = _shape_pool2.default.clone(scoped_bm_rt.v);
    }
    return scoped_bm_rt;
  }
